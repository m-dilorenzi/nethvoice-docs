---
title: Proxy Bypass for Trunks without registration
sidebar_position: 1
tags: 
  - proxy
  - trunks
...
This procedure is only necessary if an unregistered VoIP trunk (e.g., an IP-based trunk without SIP registration credentials) is not functioning correctly when routed through the NethVoice Proxy instance.
...
### 3. Creating the Bypass Rule

This rule instructs the NethVoice Proxy to bypass its own handling for a specific prefix and route the traffic directly to the chosen NethVoice instance.

1.  Click the **Add rule** button.
2.  The **Add rule** dialog will appear.
3.  Configure the rule:
    * **Root:** Enter a **unique prefix** that will act as the trigger for this bypass rule.
    ...
    * **Destination application:** Select the NethVoice instance that contains the VoIP trunk you want to bypass (e.g., `nethvoice1 [10.5.4.1]`).
4.  Click **Save**.
...
2.  The final and crucial step is to configure your NethVoice instance's PBX (FreePBX/Asterisk) to handle calls using this bypass prefix:
